...
namespace WindowsGame1
{
    public class Grid
    {
        ...
        public Grid(Rectangle screen, int numCellsLong, int numCellsHigh)
        {
            ...
            brickSrcRect = new Rectangle(65, 33, 31, 11);
            initGrid();
        }
        ...
        public void draw(SpriteBatch s, Texture2D image)
        {
            for (int i = 0; i < numCellsLong; i++)
            {
                for (int j = 0; j < numCellsHigh; j++)
                {
                    ...
                    {
                        s.Begin();
                        s.Draw(image, new Rectangle((int)(i*cellWidth), (int)(j*cellHeight), (int)cellWidth, (int)cellHeight), brickSrcRect, Color.White);
                        s.End();
                    }
                }
            }
